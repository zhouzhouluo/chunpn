...
import org.androidpn.client.XmppManagerex;
 ...
 * @author Matt Tucker
 */
public class XMPPConnectionex extends Connection {
    ...
    PacketWriterex packetWriter;
    PacketReaderex packetReader;
     ...
     * {@link #XMPPConnectionex(ConnectionConfiguration)} constructor.<p>
     * <p/>
     ...
     */
    public XMPPConnectionex(String serviceName, CallbackHandler callbackHandler) {
    ...
    /**
     * Creates a new XMPP conection in the same way {@link #XMPPConnectionex(String,CallbackHandler)} does, but
     ...
     * @param serviceName the name of the XMPP server to connect to; e.g. <tt>example.com</tt>.
     */
    public XMPPConnectionex(String serviceName) {
        // Create the configuration for this new connection
        super(new ConnectionConfiguration(serviceName));
        config.setCompressionEnabled(false);
        config.setSASLAuthenticationEnabled(true);
    ...
    /**
     * Creates a new XMPP conection in the same way {@link #XMPPConnectionex(ConnectionConfiguration,CallbackHandler)} does, but
     * with no callback handler for password prompting of the keystore.  This will work
     * in most cases, provided the client is not required to provide a certificate to 
     * the server.
     *
     ...
     */
    public XMPPConnectionex(ConnectionConfiguration config) {
        super(config);
    }
     ...
     * {@link #XMPPConnectionex(String)} constructor is a better approach.<p>
     * <p/>
     * Note that XMPPConnection constructors do not establish a connection to the server
     * and you must call {@link #connect()}.<p>
     * <p/>
     ...
     * @param callbackHandler the CallbackHandler used to prompt for the password to the keystore.
     */
    public XMPPConnectionex(ConnectionConfiguration config, CallbackHandler callbackHandler) {
        super(config);
        config.setCallbackHandler(callbackHandler);
    }
    ...
    private void initConnection() throws XMPPException {
        ...
        try {
            if (isFirstInitialization) {
                packetWriter = new PacketWriterex(this);
                packetReader = new PacketReaderex(this);
	...
	public void startKeepAliveThread(XmppManagerex xmppManager) throws Exception{
